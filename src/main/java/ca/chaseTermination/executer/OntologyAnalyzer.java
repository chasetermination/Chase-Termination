...
import ca.chaseTermination.engine.Database;
import ca.chaseTermination.engine.InMemoryDatabase;
import ca.chaseTermination.engine.PersistantDatabase;
import ca.chaseTermination.engine.Program;
import ca.chaseTermination.graph.Edge;
import ca.chaseTermination.graph.FCCNode;
import ca.chaseTermination.graph.FCComponent;
...
import static ca.chaseTermination.executer.SyntacticAnalyzer.*;
import static ca.chaseTermination.executer.TerminationAnalyzer.terminatesLinear;
...
public class OntologyAnalyzer {
    ...
    public static final String ARITY_ONE_PREDICATES = "arity_one_predicates";
    public static final String ARITY_TWO_PREDICATES = "arity_two_predicates";
    ...
    public static final String TIME_TERMINATES = "t_terminate";
    public static final String TIME_FIND_SHAPES = "t_shapes";

    public static final String TIME_LINEAR_DB = "t_linear_db";

    public static final String TIME_LINEAR_RULES = "t_linear_rules";
    ...
    public static void exportResults(FileWriter out, Map<String, Object> result, String postfix) throws IOException {
        List<String> keys = new ArrayList<>(result.keySet());
        Collections.sort(keys);
        for (String key : keys)
            out.write(key + (postfix.equals("")? "" : "-" + postfix) + ": " + result.get(key) + "\n");
    }
    ...
    private static void computeAdditionalStats(Program program) {
        Float parsingTime = (float) 0;
        if (program.stats.containsKey(TIME_PARSING))
            parsingTime = (Float) program.stats.get(TIME_PARSING);
        program.stats.put(TIME_TERMINATES,
                parsingTime +
                        ((Float) program.stats.get(TIME_GENERATE_DEP_GRAPH)) +
                        ((Float) program.stats.get(TIME_CONNECTED_COMPONENT)));
    }

    public static void analyzeProgramTermination(Program program, boolean simple) throws SQLException {
    ...
    public static void addProgramStats(Program program, Map<Position, Node> graph) {
        ...
        int sum = 0, count = 0, ar_one = 0, ar_two = 0;
        ...
        program.stats.put(ARITY_ONE_PREDICATES, ar_one);
        program.stats.put(ARITY_TWO_PREDICATES, ar_two);
    ...
    public static void main(String[] args) throws Exception {
        ...
        boolean simple = !Execute.checkOption(args, "-l");

        if (!program.isLinear()) {
            if (Execute.checkOption(args, "-ln"))
        ...
        exportResults(out, program.stats, "");
