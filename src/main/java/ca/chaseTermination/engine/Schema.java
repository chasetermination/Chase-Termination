...
import ca.chaseTermination.primitives.Constant;
import ca.chaseTermination.primitives.Predicate;
import ca.chaseTermination.synthesizer.ProgramGenerator;
...
public class Schema {
    ...
    public Constant fetchConstant(String label) {
        if (constants.containsKey(label)) return constants.get(label);
    ...
    public Predicate fetchPredicate(String name, int arity) {
        ...
        if (!predicates.containsKey(name)) {
            ...
            if (predicate.arity != arity) throw new RuntimeException("Invalid Arity! " + predicate);
        }
        return predicate;
    }

    public Predicate fetchSimplePredicate(Predicate predicate) {
        if (!predicate.isAdorned()) return null;
